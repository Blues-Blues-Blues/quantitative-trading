...
- **连续评分信号**：ES（入场分，sigmoid 合成资金主体/纯净度/量价共振）+ PS（持仓分 = ES×Time_Decay×Fund_Stability）+ XS（出局分 clip 加权），数值统一有界化；独立 A 股硬过滤层（ST 禁买、涨跌停禁买卖、时间窗 09:45~14:50、成交额门槛）前置否决 + 一票否决（游资溃逃 / 大盘跳水），全部评分参数可寻优
...
- 2 只 × 2 年约 3 分钟；20 只 × 4 个月约 5~6 分钟（全量 2 年会更久）
...
- **持仓 XS 三分支**：出局（XS ≤ th_xs_exit）→ 目标是 0（清仓）；减仓（th_xs_exit < XS < th_xs_reduce）→ 阶梯减仓（`simulated_weight × reduce_step_ratio`，默认 ×0.8）；持有/加仓（XS ≥ th_xs_reduce）→ 目标 = `base × PS × 乘子`
...
- **PS 持仓分**：`ES × Time_Decay × Fund_Stability`；Time_Decay = 0.95^(bars_held/10)，浮盈超 momentum_exempt 时动量豁免不减分
- **XS 出局分**：汇入均线偏离、动量衰减、回撤（高水位）、超时等维度，clip 加权到 [-1,1]，驱动 清仓 / 减仓 / 持有 三分支
- **A 股硬过滤层**（否决前置）：ST 禁买、涨停禁买/跌停禁卖、交易时间窗 09:45~14:50、成交额门槛；未持仓时任一不过即不开仓（`hard_filters` 与状态机 BUY 口径一致）
...
- **T+1 顺延**：当日买入份额锁定不可卖，SELL/减仓遇锁定挂入 `pending_targets` 逐 Bar 顺延，按当前价换算后重试（跌停日跳过但挂起不丢失）；盘中涨停不可买入、跌停不可卖出；100 股整数倍取整
...
当前 164 项单元测试全部通过（合成 mock 数据，不联网；运行约 40 秒）：
